...
class SharedDataManager {
    ...
    private let appGroupIdentifier = "group.com.yourname.TodoApp.shared"
...
class SimpleTodoManager: ObservableObject {
    ...
    /// サンプルTodoを追加する（デバッグ用）
    func addSampleTodos() {
        addTodo(text: "朝のランニング")
        addTodo(text: "友達と映画を見る")
        addTodo(text: "新しいレシピを試す")
    }
    ...
    private func loadSampleData() {
        let sampleTodos = [
            TodoItem(id: "1", text: "プロジェクトの仕様書を確認", isCompleted: false, lineNumber: 1, originalLine: "- [ ] プロジェクトの仕様書を確認"),
            TodoItem(id: "2", text: "UIのワイヤーフレームを作成", isCompleted: true, lineNumber: 2, originalLine: "- [x] UIのワイヤーフレームを作成"),
            TodoItem(id: "3", text: "データベース設計を検討", isCompleted: false, lineNumber: 3, originalLine: "- [ ] データベース設計を検討")
        ]
...
struct ContentView: View {
    ...
    @ViewBuilder
    private var advertisementBanner: some View {
        VStack(spacing: 0) {
            Divider()
            ...
            NavigationStack {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    Menu {
                        ...
                        Section("デバッグ") {
                            Button {
                                todoManager.addSampleTodos()
                            } label: {
                                Label("サンプルTodo追加", systemImage: "plus.rectangle")
                            }
                        }
                    } label: {
